...
final class SpecialtyDetailRepositoryImpl implements SpecialtyDetailRepository {
  final SpecialtyDetailRemoteDataSource remoteDataSource;
  SpecialtyDetailRepositoryImpl(this.remoteDataSource);

  @override
  Future<DataState<SpecialtyDetail>> getSpecialtyDetail(String url) async {
    final detailModel = await remoteDataSource.getSpecialtyDetailFromUrl(url);
    if (detailModel is DataFailed) return DataFailed(detailModel.error!);
    return DataSuccess(detailModel.data!.toDomainEntity());
  }
}
